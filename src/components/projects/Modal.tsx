...
const Modal = ({ modal, projects }: modalProps) => {
  ...
  return (
    <>
      ...
      <motion.div
        ref={cursor}
        className="w-20 h-20 rounded-full bg-[#455CE9] text-white absolute z-2 flex items-center justify-center text-sm font-normal font-poppins pointer-events-none"
        variants={scaleAnimation}
        initial="initial"
        animate={active ? "enter" : "closed"}
      ></motion.div>
      <motion.div
        ref={cursorLabel}
        className="absolute flex items-center justify-center w-20 h-20 text-sm font-normal text-white bg-transparent rounded-full pointer-events-none z-2 font-poppins"
        variants={scaleAnimation}
        initial="initial"
        animate={active ? "enter" : "closed"}
      >
